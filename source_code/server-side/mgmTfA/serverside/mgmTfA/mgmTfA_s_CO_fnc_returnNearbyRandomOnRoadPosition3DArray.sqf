//H
//H ~~
//H $FILE$		:	<mission>/custom/mgmTfA/mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf
...
//HH
//HH ~~
//HH	Syntax		:	Array = mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray RADIUS(number), ORIGIN MARKER(string - "MarkerName")
//HH	Parameters	:
...
//HH	Example usage	:	_outputPosArray = [50, [18396.9,14253.7]] call mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray;
//HH ~~
//HH
...
scopeName "mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArrayMainScope";
private	[
		"_randomPickRadius",
...
if (typeName _randomPickRadius != "SCALAR") exitWith {
		...
		diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [ERROR] First parameter (RADIUS) must be a number! (Received: %1)", _typeResult];//dbg
		_ranPos;
};

// _randomPickRadius must be provided and must be greater than zero
if (!(_randomPickRadius >0)) exitWith {
		diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [ERROR] _randomPickRadius is nil! (%1). It must be provided as parameter0. Terminating function. Returning empty array.", _randomPickRadius];//dbg
		_ranPos;
};
	if (_thisFileVerbosityLevel>=4) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [TV4] Successfully validated argument _randomPickRadius. _randomPickRadius value is: (%1)", _randomPickRadius]; };//dbg
// ~~~~~~~~~~
...
if (typeName _minDistanceInMeters != "SCALAR") exitWith {
	...
	diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [ERROR] Second parameter (MIN. DISTANCE) must be a number! (Received: %1)", _typeResult];//dbg
	_ranPos;
};

// _minDistanceInMeters must be provided and must be greater than zero
if (!(_minDistanceInMeters >0)) exitWith {
	diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [ERROR] _minDistanceInMeters is nil! (%1). It must be provided as parameter1. Terminating function. Returning empty array.", _minDistanceInMeters];//dbg
	_ranPos;
};
	if (_thisFileVerbosityLevel>=4) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [TV4] Successfully validated argument _minDistanceInMeters. _minDistanceInMeters value is: (%1)", _minDistanceInMeters]; };//dbg
// ~~~~~~~~~~
...
if (isnil "_originalPositionArray2D") exitWith {diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [ERROR] _originalPositionArray2D is nil! It must be passed as parameter2 and must contain ""Array - format Position2D"" (e.g.: [13284.4,14571.6]). Terminating function. Returning empty array."];};//dbg
if (count _originalPositionArray2D == 0) exitWith {
	diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [ERROR] _originalPositionArray2D is empty! It must be passed as parameter2 and must contain ""Array - format Position2D"" (e.g.: [13284.4,14571.6]). Terminating function. Returning empty array."];//dbg
	_ranPos;
};
	if (_thisFileVerbosityLevel>=4) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [TV4] Successfully validated argument _originalPositionArray2D. _originalPositionArray2D value is: (%1)", _originalPositionArray2D];};//dbg
// ~~~~~~~~~~
...
_distance717 = random _randomPickRadius;
	if (_thisFileVerbosityLevel>=4) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [TV4] random _direction717 is %1. random _distance717 is %2.", _direction717, _distance717]; };//dbg
// generate the random position
_currentAttemptNumber = 0;
_numberOfAllowedMaxAttempts = 99;
	if (_thisFileVerbosityLevel>=4) then {
		diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [TV4] _currentAttemptNumber is 1. _numberOfAllowedMaxAttempts is %1", _numberOfAllowedMaxAttempts];//dbg
		_originX = (_originalPositionArray2D select 0);
		_originY = (_originalPositionArray2D select 1);
		diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [TV4] originX is: (%1)     originY is: (%2)", _originX, _originY];//dbg
	};
_ranPos = [(_originalPositionArray2D select 0) + _distance717 * sin _direction717, (_originalPositionArray2D select 1) + _distance717 * cos _direction717, 0];
	if (_thisFileVerbosityLevel>=4) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [TV4] _ranPos is %1.", _ranPos];};//dbg
...
while {((_isWater) || (count _isHouse > 0) || (!_isOnRoad) || (!_fartherThanMinDistanceBool))} do {
	...
	if (_currentAttemptNumber > _numberOfAllowedMaxAttempts) then {
		diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [TV4] Consumed numberOfAllowedMaxAttempts (%1). Could not find a suitable RanPos within the radius (%2 metres) of origin (%3). Panic Mode: on!.", _numberOfAllowedMaxAttempts, _randomPickRadius, _originalPositionArray2D];//dbg
		_iAmInPanic = true;
		breakTo "mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArrayMainScope";
	};
	...
	if (_thisFileVerbosityLevel>=4) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [TV4] Previous RanPos was not matching the active search criteria. Created a new RanPos (this was attemptNo=%1). New RanPos is at: %2. Whether it matches the criteria will be checked shortly.", _currentAttemptNumber, _ranPos];};//dbg
};

if (_iAmInPanic) then {
	// not good
	if (_thisFileVerbosityLevel>=1) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [TV4] PANIC MODE INITIALIZING..."];};//dbg
...
if (mgmTfA_configgv_serverVerbosityLevel>=3) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_fnc_returnNearbyRandomOnRoadPosition3DArray.sqf] [TV2] Reached checkpoint: Bottom of function. Returning now. _ranPos array contain: (%1)", (str _ranPos)];};//dbg

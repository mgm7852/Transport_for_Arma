...
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase02aa_SendResponse_ChargeTaxiAnywhereInitialBookingFeeRequestActioned.sqf]  [TV4] TaxiAnywhere Initial Booking Fee Charge request received. This is what I have received:		_clickNGoRequestorClientIDNumber: (%1).		_clickNGoRequestorPlayerUIDTextString: (%2).		_clickNGoRequestorProfileNameTextString: (%3)", _clickNGoRequestorClientIDNumber, _clickNGoRequestorPlayerUIDTextString, _clickNGoRequestorProfileNameTextString];};//dbg
...
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase02aa_SendResponse_ChargeTaxiAnywhereInitialBookingFeeRequestActioned.sqf]  [TV4] SENT RESPONSE    (mgmTfA_gv_pvc_pos_yourclickNGoPAYGInitialBookingFeeChargeRequestActionedPacketSignalOnly) to Requestor:  (%1),		on computer (_clickNGoRequestorClientIDNumber)=(%2).", _clickNGoRequestorProfileNameTextString, _clickNGoRequestorClientIDNumber];};//dbg
if (_thisFileVerbosityLevelNumber>=5) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase02aa_SendResponse_ChargeTaxiAnywhereInitialBookingFeeRequestActioned.sqf]  [TV5] EXITING SCRIPT NOW."];};//dbg
// EOF
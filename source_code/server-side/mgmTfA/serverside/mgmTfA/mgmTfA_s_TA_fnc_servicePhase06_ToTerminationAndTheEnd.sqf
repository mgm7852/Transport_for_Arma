//H
//H ~~
//H $FILE$		:	<mission>/custom/mgmTfA/mgmTfA_s_TA_fnc_servicePhase06_ToTerminationAndTheEnd.sqf
...
scopeName "mgmTfA_s_TA_fnc_servicePhase06_ToTerminationAndTheEndMainScope";
...
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_s_TA_fnc_servicePhase06_ToTerminationAndTheEnd.sqf]  [TV3] I have been SPAWN'd.	This is full raw dump of what I received:	(%1).", (str _this)];};//dbg
if (!isNil "_emergencyEscapeNeeded") then {
	if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_s_TA_fnc_servicePhase06_ToTerminationAndTheEnd.sqf]  [TV3] (str _emergencyEscapeNeeded) is: (%1).", (str _emergencyEscapeNeeded)];};//dbg
};
...
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_s_TA_fnc_servicePhase06_ToTerminationAndTheEnd.sqf]  [TV3] Distance to Waypoint _SUTerminationPointPosition3DArray is: (%1). Going there now.", _SUTaxiAIVehicleDistanceToWayPointMetersNumber];};//dbg
...
while {_SUTaxiAIVehicleDistanceToWayPointMetersNumber>25} do {
	...
	if(_emergencyEscapeNeeded) then {	breakTo "mgmTfA_s_TA_fnc_servicePhase06_ToTerminationAndTheEndMainScope";	};
	...
	if (fuel _SUTaxiAIVehicleObject < 0.2) then {
		...
		if (_thisFileVerbosityLevelNumber>=2) then {diag_log format ["[mgmTfA] [mgmTfA_s_TA_fnc_servicePhase06_ToTerminationAndTheEndMainScope.sqf]  [TV2] REFUELing SU Vehicle: (%1) | Driver: (%2) | ServerUpTime: (%3)", _myGUSUIDNumber, _SUDriversFirstnameTextString, (round (time))];};//dbg
	};
	// Pit-stop checks: AutoRepair
	if (damage _SUTaxiAIVehicleObject>0.2) then {
		_SUTaxiAIVehicleObject setDamage 0;
		if (_thisFileVerbosityLevelNumber>=2) then {diag_log format ["[mgmTfA] [mgmTfA_s_TA_fnc_servicePhase06_ToTerminationAndTheEndMainScope.sqf]  [TV2] REPAIRing SU Vehicle: (%1) | Driver: (%2) | ServerUpTime: (%3)", _myGUSUIDNumber, _SUDriversFirstnameTextString, (round (time))];};//dbg
	};
};
...
waitUntil {speed _SUTaxiAIVehicleObject == 0};
	if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_s_TA_fnc_servicePhase06_ToTerminationAndTheEnd.sqf]  [TV4]          TERMINATION SEQUENCE IN PROGRESS: Waiting for full-stop before deleting driver & vehicle."];};//dbg
...
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_s_TA_fnc_servicePhase06_ToTerminationAndTheEnd.sqf]  [TV4]          ENTERed TERMINATION SLEEP LOOP."];};//dbg
_broadcastSUInformationCounter = 0;
...
if (mgmTfA_configgv_taxiAnywhereTaxisSpinBeforeDeletionBool) then {
	...
	if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_s_TA_fnc_servicePhase06_ToTerminationAndTheEnd.sqf]  [TV4] <ThisIs:%1> TERMINATION SEQUENCE IN PROGRESS: Just completed DeletionRotation on the vehicle.", _myGUSUIDNumber];};//dbg
};
...
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_s_TA_fnc_servicePhase06_ToTerminationAndTheEnd.sqf]  [TV3]          TERMINATION SEQUENCE IN PROGRESS: Issued: mgmTfA_gv_PV_SU%1SUMarkerShouldBeDestroyedAfterExpiryBool! for GUSUID: (%1)", _myGUSUIDNumber];};//dbg

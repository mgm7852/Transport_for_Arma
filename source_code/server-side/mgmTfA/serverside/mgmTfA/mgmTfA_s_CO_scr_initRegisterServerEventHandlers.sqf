...
"mgmTfA_gv_pvs_req_clickNGoTaxiToMyPositionPleaseConfirmPacket" addPublicVariableEventHandler {
	...
	scopeName "_PUIDsAndPlayernamesTextStringArrayCountNumberGreaterThanZeroScope";
		...
		// If PUID  matches, we will return the profileName
		{
			...
			if (_clickNGoRequestorPlayerUIDTextString == (_x select 0)) then {
				...
				if (mgmTfA_configgv_serverVerbosityLevel>=4) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_scr_initRegisterServerEventHandlers.sqf]  [TV3]          _clickNGoRequestorProfileNameTextString is now set to: (%1). issuing (breakTo _PUIDsAndPlayernamesTextStringArrayCountNumberGreaterThanZeroScope) now.", (_x select 1)];};//dbg
				breakTo "_PUIDsAndPlayernamesTextStringArrayCountNumberGreaterThanZeroScope";
			};
		}  forEach mgmTfA_pvdb_PUIDsAndPlayernamesTextStringArray;
	};
	...
	// Is the Requestor Blacklisted Check
	if (mgmTfA_configgv_serverVerbosityLevel>=3) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_scr_initRegisterServerEventHandlers.sqf]  [TV3]           Will start traversing (mgmTfA_dynamicgv_clickNGoTaxisBlacklistedPlayerPUIDsTextStringArray) in the next line."];};//dbg
	{
		...
		if (_clickNGoRequestorPlayerUIDTextString == _x) then {
			if (mgmTfA_configgv_serverVerbosityLevel>=4) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_scr_initRegisterServerEventHandlers.sqf]  [TV4]           Player is in _clickNGoRequestorIsInBlacklist! He is blacklisted! He cannot use the clickNGo Taxi service!"];};
	...
	if (mgmTfA_configgv_serverVerbosityLevel>=3) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_scr_initRegisterServerEventHandlers.sqf]  [TV3]           We are just outside `forEach` loop. The value for _clickNGoRequestorIsInBlacklist is: (%1).", (str _clickNGoRequestorIsInBlacklist)];};
	// We have done the checks above. Now let's proceed accordingly.
	if(_clickNGoRequestorIsInBlacklist) then {
		...
		_null = [_clickNGoRequestorClientIDNumber, _clickNGoRequestorPosition3DArray, _clickNGoRequestorPlayerUIDTextString, _clickNGoRequestorProfileNameTextString, _clickNGoTaxiRequestedDestinationPosition3DArray] spawn mgmTfA_s_TA_fnc_servicePhase02b_SendResponse_BookingRequestRejected_RequestorIsInBlacklist;
	} else {
		// Report to log
		...
		_null = [_clickNGoRequestorClientIDNumber, _clickNGoRequestorPosition3DArray, _clickNGoRequestorPlayerUIDTextString, _clickNGoRequestorProfileNameTextString, _clickNGoTaxiRequestedDestinationPosition3DArray] spawn mgmTfA_s_TA_fnc_servicePhase02a_SendResponse_BookingRequestAccepted;
	};
};
"mgmTfA_gv_pvs_req_clickNGoTaxiChargeMePAYGTickCostPleaseConfirmPacket" addPublicVariableEventHandler {
	...
	if (mgmTfA_configgv_serverVerbosityLevel>=3) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_scr_initRegisterServerEventHandlers.sqf]      [PAYG CHARGE ME TICK COST]    RECEIVED chargeMeTickCost REQUEST    _clickNGoRequestorClientIDNumber: (%1).	_clickNGoRequestorProfileNameTextString: (%2).		_clickNGoRequestorPlayerUIDTextString: (%3).", _clickNGoRequestorClientIDNumber, _clickNGoRequestorProfileNameTextString, _clickNGoRequestorPlayerUIDTextString];};//dbg
	// STAGE IN WORKFLOW:		Action the request = Charge the player
	_null = [_requestorPlayerObject, mgmTfA_configgv_clickNGoTaxisTickCostInCryptoNegativeNumber] call EPOCH_exp_server_effectCrypto;
	// Report to log
	...
	_null = [_clickNGoRequestorClientIDNumber, _clickNGoRequestorPlayerUIDTextString, _clickNGoRequestorProfileNameTextString] spawn mgmTfA_s_TA_fnc_servicePhase04b_SendResponse_ChargePAYGTickCostRequestActioned;
};
"mgmTfA_gv_pvs_req_TAChargeMe1stMileFeePacket" addPublicVariableEventHandler {
	...
	_null = [_clickNGoRequestorClientIDNumber, _clickNGoRequestorPlayerUIDTextString, _clickNGoRequestorProfileNameTextString] spawn mgmTfA_s_TA_fnc_servicePhase04a_SendResponse_Charge1stMileFeeRequestActioned;
};
...
"mgmTfA_gv_pvs_req_clickNGoTaxiChargeMeInitialBookingFeePleaseConfirmPacket" addPublicVariableEventHandler {
	...
	if (mgmTfA_configgv_serverVerbosityLevel>=3) then {diag_log format ["[mgmTfA] [mgmTfA_s_CO_scr_initRegisterServerEventHandlers.sqf]      [PAYG CHARGE ME INITIAL BOOKING FEE ]    RECEIVED chargeMeInitialBookingFee REQUEST    _clickNGoRequestorClientIDNumber: (%1).	_clickNGoRequestorProfileNameTextString: (%2).		_clickNGoRequestorPlayerUIDTextString: (%3).", _clickNGoRequestorClientIDNumber, _clickNGoRequestorProfileNameTextString, _clickNGoRequestorPlayerUIDTextString];};//dbg
	// STAGE IN WORKFLOW:		Action the request
	_null = [_requestorPlayerObject, mgmTfA_configgv_clickNGoTaxisNonRefundableBookingFeeCostInCryptoNegativeNumber] call EPOCH_exp_server_effectCrypto;
	// Report to log
	...
	_null = [_clickNGoRequestorClientIDNumber, _clickNGoRequestorPlayerUIDTextString, _clickNGoRequestorProfileNameTextString] spawn mgmTfA_s_TA_fnc_servicePhase02aa_SendResponse_ChargePAYGInitialBookingFeeRequestActioned;
};

...
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_ClickNGoTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf] [TV4] EXITED LOOP: DrivingToPickUpPointWhileLoop.	Next, SPAWN'ing: (mgmTfA_fnc_server_clickNGoTaxi_ServicePhase04_PickUpPointAndBeyond)"];};//dbg
...
if (!_emergencyEscapeNeeded) then { 
	...
	if (_thisFileVerbosityLevelNumber>=2) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_ClickNGoTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf] [TV2] <<<reached end-of-file>>>.   no emergency. proceeding with normal next phase in the workflow.			SPAWN'ing (mgmTfA_fnc_server_clickNGoTaxi_ServicePhase04_PickUpPointAndBeyond)."];};//dbg

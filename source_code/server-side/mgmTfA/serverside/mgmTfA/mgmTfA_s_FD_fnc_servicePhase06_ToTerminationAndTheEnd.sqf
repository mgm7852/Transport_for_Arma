...
mgmTfA_dgv_fixedDestinationTaxisTotalDistanceTravelledByTaxisNumber = mgmTfA_dgv_fixedDestinationTaxisTotalDistanceTravelledByTaxisNumber + _iWantToTravelThisManyMetresNumber;
	if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_s_FD_fnc_servicePhase06_ToTerminationAndTheEnd.sqf]  [TV4] mgmTfA_dgv_fixedDestinationTaxisTotalDistanceTravelledByTaxisNumber is now (%1). It now reflects the distance I just travelled (%2).]", mgmTfA_dgv_fixedDestinationTaxisTotalDistanceTravelledByTaxisNumber, _iWantToTravelThisManyMetresNumber];};//dbg
...
//	TODO	THIS IS NOT READY. FIRST FINISH BASIC WORKFLOW. THEN COME FIX THIS.			[false] call PublicVariableBroadcastSUInformation;
	...
	if (_thisFileVerbosityLevelNumber>=10) then {diag_log format ["[mgmTfA] [mgmTfA_s_FD_fnc_servicePhase06_ToTerminationAndTheEnd.sqf]  [TV10]          TICK in TERMINATION point, _delayedDeletion SLEEP LOOP."];};//dbg
};
if (_thisFileVerbosityLevelNumber>=9) then {diag_log format ["[mgmTfA] [mgmTfA_s_FD_fnc_servicePhase06_ToTerminationAndTheEnd.sqf]  [TV9]          EXITed TERMINATION point, _delayedDeletion SLEEP LOOP."];};//dbg

//H
//H ~~
//H $FILE$		:	<mission>/custom/mgmTfA/mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase02a_SendResponse_BookingRequestAccepted.sqf
...
//RELEASE TODO
//_positionToSpawnSUVehiclePosition3DArray=[700,500, _fixedDestinationRequestorPosition3DArray] call mgmTfA_fnc_server_returnNearbyRandomOnRoadPosition3DArray;
//_positionToSpawnSUVehiclePosition3DArray=[120,80, _fixedDestinationRequestorPosition3DArray] call mgmTfA_fnc_server_returnNearbyRandomOnRoadPosition3DArray;
_positionToSpawnSUVehiclePosition3DArray=[mgmTfA_configgv_fixedDestinationTaxisSpawnDistanceRadiusInMetresNumber,mgmTfA_configgv_fixedDestinationTaxisSpawnDistanceRadiusMinDistanceInMetresNumber, _fixedDestinationRequestorPosition3DArray] call mgmTfA_fnc_server_returnNearbyRandomOnRoadPosition3DArray;
...
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase02a_SendResponse_BookingRequestAccepted.sqf]  [TV3] _positionToSpawnSUVehiclePosition3DArray random position is randomly chosen as (%1)", _positionToSpawnSUVehiclePosition3DArray];};
...
// Broadcast the value to all computers: this is done ONLY (during  startup, in server-side init) && (after the server startup, it is done only in: mgmTfA_fnc_server_PublicVariableBroadcastSUInformationInitialBroadcast.sqf)
// Note that "mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf"	will call "mgmTfA_fnc_server_PublicVariableBroadcastSUInformationInitialBroadcast"
...
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase02a_SendResponse_BookingRequestAccepted.sqf]  [TV3] <ThisIs:%1> mgmTfA_gvdb_PV_GUSUIDNumber has been incremented; new value is: (%2)", _myGUSUIDNumber, mgmTfA_gvdb_PV_GUSUIDNumber];};//dbg
...
// Proceed to next Phase:		Service Unit Creation		"mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnit"
...
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase02a_SendResponse_BookingRequestAccepted.sqf]  [TV3] <ThisIs:%1> I have spawn'd `mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnit`. I'm quitting now.", _myGUSUIDNumber];};
// EOF
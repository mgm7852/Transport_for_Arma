//H
//H ~~
//H $FILE$		:	<mission>/custom/mgmTfA/mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf
...
scopeName "mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEndMainScope";
...
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV3] I have been SPAWN'd.	This is full raw dump of what I received:	(%1).", (str _this)];};//dbg
if (!isNil "_emergencyEscapeNeeded") then {
	if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV3] (str _emergencyEscapeNeeded) is: (%1).", (str _emergencyEscapeNeeded)];};//dbg
};

// it is possible that we received this one in 'true' state; if we do not set it to false, taxi will never try travelling to the termination point. it will immediately destroy at its existing  spot which is bad  for immersion.
// let's set it to false. if need be [if it times out during its travel to termination point] it will be automatically set to 'true' again...
...
mgmTfA_dynamicgv_clickNGoTaxisTotalDistanceTravelledByTaxisNumber = mgmTfA_dynamicgv_clickNGoTaxisTotalDistanceTravelledByTaxisNumber + _iWantToTravelThisManyMetresNumber;
	if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV4] mgmTfA_dynamicgv_clickNGoTaxisTotalDistanceTravelledByTaxisNumber is now (%1). It now reflects the distance I just travelled (%2).]", mgmTfA_dynamicgv_clickNGoTaxisTotalDistanceTravelledByTaxisNumber, _iWantToTravelThisManyMetresNumber];};//dbg
...
_SUTerminationPointPosition3DArray = [mgmTfA_configgv_clickNGoTaxisTerminationDistanceRadiusInMetresNumber, mgmTfA_configgv_clickNGoTaxisTerminationDistanceRadiusMinDistanceInMetresNumber, _SUTaxiAIVehicleObjectCurrentPositionPosition3DArray] call mgmTfA_fnc_server_returnNearbyRandomOnRoadPosition3DArray;
...
//check distance to our Current Waypoint (_requ
...
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV4] AFTER adding the WaypointType: _SUTaxiAIVehicleWaypointMainArrayIndexNumber contains: (%1).   _SUTaxiAIVehicleWaypointMainArray contains: (%2)", (str _SUTaxiAIVehicleWaypointMainArrayIndexNumber), (str _SUTaxiAIVehicleWaypointMainArray)];};//dbg
...
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV3] Distance to Waypoint _SUTerminationPointPosition3DArray is: (%1). Going there now.", _SUTaxiAIVehicleDistanceToWayPointMetersNumber];};//dbg
// LOOP ON THE WAY TO PICKUP!
_counterForLogOnlyEveryNthPINGNumber = 0;
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [DEV-DEBUG]           NEXT, will enter drivingToTerminationPoint25."];};//dbg
...
while {_SUTaxiAIVehicleDistanceToWayPointMetersNumber>25} do {
	...
	if (_SUCurrentTaskAgeInSecondsNumber > _SUCurrentTaskThresholdInSecondsNumber) then {
		...
		//if (_thisFileVerbosityLevelNumber>=1) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV1]          Expiry Timeout Threshold Exceeded for SU (%1). Initiating Abnormal SU Termination! _SUCurrentTaskAgeInSecondsNumber is: (%2). _SUCurrentTaskThresholdInSecondsNumber is: (%3).", _myGUSUIDNumber, _SUCurrentTaskAgeInSecondsNumber, _SUCurrentTaskThresholdInSecondsNumber];};//dbg
	...
	if(_emergencyEscapeNeeded) then {	breakTo "mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEndMainScope";	};
	...
	if (fuel _SUTaxiAIVehicleObject < 0.2) then {
		...
		if (_thisFileVerbosityLevelNumber>=2) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEndMainScope.sqf]  [TV2] REFUELing SU Vehicle: (%1) | Driver: (%2) | ServerUpTime: (%3)", _myGUSUIDNumber, _SUDriversFirstnameTextString, (round (time))];};//dbg
	};
	// Pit-stop checks: AutoRepair
	if (damage _SUTaxiAIVehicleObject>0.2) then {
		_SUTaxiAIVehicleObject setDamage 0;
		if (_thisFileVerbosityLevelNumber>=2) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEndMainScope.sqf]  [TV2] REPAIRing SU Vehicle: (%1) | Driver: (%2) | ServerUpTime: (%3)", _myGUSUIDNumber, _SUDriversFirstnameTextString, (round (time))];};//dbg
	};
};
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf] [TV4] EXITED LOOP: drivingToTerminationPoint25"];};//dbg
...
_SUTaxiAIVehicleObjectAgeInSecondsNumber = (round ((time) - _SUTaxiAIVehicleObjectBirthTimeInSecondsNumber));
	...
	if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV4]          I have arrived at SELF-TERMINATION POINT."];};//dbg
	if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV4]          TERMINATION Sequence in Progress for Service Unit:          Driver:(%1)   |   in Vehicle: (%2)   |   ServerUpTime: (%3)   |   MyAge: (%4).",  _SUDriversFirstnameTextString, _myGUSUIDNumber, (round (time)), _SUTaxiAIVehicleObjectAgeInSecondsNumber];};//dbg
...
waitUntil {speed _SUTaxiAIVehicleObject == 0};
	if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV4]          TERMINATION SEQUENCE IN PROGRESS: Waiting for full-stop before deleting driver & vehicle."];};//dbg
...
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV4]          ENTERed TERMINATION SLEEP LOOP."];};//dbg
_broadcastSUInformationCounter = 0;
...
//	TODO	THIS IS NOT READY. FIRST FINISH BASIC WORKFLOW. THEN COME FIX THIS.			[false] call PublicVariableBroadcastSUInformation;
	...
	if (_thisFileVerbosityLevelNumber>=5) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV5]          TICK in TERMINATION point, _delayedDeletion SLEEP LOOP."];};//dbg
};
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV4]          EXITed TERMINATION point, _delayedDeletion SLEEP LOOP."];};//dbg
...
if (mgmTfA_configgv_clickNGoTaxisSpinBeforeDeletionBool) then {
	...
	if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV4] <ThisIs:%1> TERMINATION SEQUENCE IN PROGRESS: Just completed DeletionRotation on the vehicle.", _myGUSUIDNumber];};//dbg
};
// Delete the driver now
deleteVehicle _SUAICharacterDriverObject;
// Delete the vehicle now
deleteVehicle _SUTaxiAIVehicleObject;
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV4] <ThisIs:%1> TERMINATION SEQUENCE IN PROGRESS: Deleted driver & vehicle.", _myGUSUIDNumber];};//dbg
...
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV4]          TERMINATION SEQUENCE IN PROGRESS: mgmTfA_gvdb_PV_clickNGoTaxisNumberOfCurrentlyAvailableTaxiDriversNumber is (%1) BEFORE the change.", mgmTfA_gvdb_PV_clickNGoTaxisNumberOfCurrentlyAvailableTaxiDriversNumber];};//dbg
...
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV4]          TERMINATION SEQUENCE IN PROGRESS: mgmTfA_gvdb_PV_clickNGoTaxisNumberOfCurrentlyAvailableTaxiDriversNumber is (%1) AFTER the change.", mgmTfA_gvdb_PV_clickNGoTaxisNumberOfCurrentlyAvailableTaxiDriversNumber];};//dbg
...
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV3]          TERMINATION SEQUENCE IN PROGRESS: Issued: mgmTfA_gv_PV_SU%1SUMarkerShouldBeDestroyedAfterExpiryBool! for GUSUID: (%1)", _myGUSUIDNumber];};//dbg
...
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV3]          I have SUCCESSFULLY FULFILLED a request therefore incremented & publicVariable broadcasted (mgmTfA_gvdb_PV_clickNGoTaxisTotalRequestsReceivedNumber) by one. Current value, after the increment, is: (%1). The requestor for this successful job was: (%2).", mgmTfA_gvdb_PV_clickNGoTaxisTotalRequestsReceivedNumber, _clickNGoRequestorProfileNameTextString];};//dbg
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEnd.sqf]  [TV3]          TERMINATION SEQUENCE COMPLED. Have a nice day."];};//dbg
if (_thisFileVerbosityLevelNumber>=2) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_clickNGoTaxi_ServicePhase06_ToTerminationAndTheEndMainScope.sqf]  [TV2] <<<reached end-of-file>>>"];};//dbg
// EOF
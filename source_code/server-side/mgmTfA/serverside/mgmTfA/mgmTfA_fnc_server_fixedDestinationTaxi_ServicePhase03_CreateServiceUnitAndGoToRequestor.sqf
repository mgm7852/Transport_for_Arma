//H
//H ~~
//H $FILE$		:	<mission>/custom/mgmTfA/mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf
...
scopeName "mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestorMainScope";
private	[
...
if (mgmTfA_dynamicgv_sideRelationSetupHasNotBeenDoneYetBool) then {
	if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf]  [TV4] Sides DO NEED set up -- I will do it now. I will set mgmTfA_dynamicgv_sideRelationSetupHasNotBeenDoneYetBool to false."];};//dbg
	...
	if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf]  [TV4] Sides DO NOT NEED set up -- nothing to be done on this."];};//dbg
};
...
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf]  [TV3]           Waypoint Added: %2 at %1", _fixedDestinationRequestorPosition3DArray, _SUTaxiAIVehicleWaypointMainArray];};//dbg
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf]  [TV3]           Waypoints Added: The current Waypoint Array is: (%1). _SUTaxiAIVehicleWaypointMainArrayIndexNumber is: (%2)",_SUTaxiAIVehicleWaypointMainArray, _SUTaxiAIVehicleWaypointMainArrayIndexNumber];};//dbg
...
if (mgmTfA_configgv_currentMod == "EPOCH") then {
	...
	if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf]  [TV3]           Server is running EPOCH MOD. Post vehicle creation, setVToken issued to prevent accidental-clean up."];};//dbg
};
...
for [{_n=1}, {_n <2}, {_n=_n+1;}] do
{
		...
		_SUDriversFirstnameTextString = [] call mgmTfA_fnc_server_returnARandomFirstnameTextString;
		_SUAICharacterDriverObject setVariable ["firstName",_SUDriversFirstnameTextString,true];
			if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf]  [TV3]           Just picked a random name, here is the result... _SUDriversFirstnameTextString is set to: (%1)", _SUDriversFirstnameTextString];};//dbg
...
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf]  [TV3] SPAWN Completed. Vehicle: (%1) | GUSUID: (%2) | Driver: (%3)", _SUTaxiAIVehicleObject, _myGUSUIDNumber, _SUDriversFirstnameTextString];};//dbg
...
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf]  [TV3] current value:   (_iWantToTravelThisManyMetresNumber)=(%1)   ]", _iWantToTravelThisManyMetresNumber];};//dbg
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf]  [TV4] SLEEPing now for %1 seconds before locking doors.", mgmTfA_configgv_timeToSleepBeforeLockingSpawnedFixedDestinationTaxiVehicleDoors];};//dbg
...
if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf]  [TV4] Distance to Waypoint _fixedDestinationRequestorPosition3DArray is: (%1). Locked the doors & going there now.", _SUTaxiAIVehicleDistanceToWayPointMetersNumber];};//dbg
...
while {(_SUTaxiAIVehicleDistanceToWayPointMetersNumber>10) && ((speed _SUTaxiAIVehicleObject) != 0)} do {
	...
	if (_SUCurrentTaskAgeInSecondsNumber > _SUCurrentTaskThresholdInSecondsNumber) then {
		...
		breakTo "mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestorMainScope";
	};
...
if (_thisFileVerbosityLevelNumber>=4) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf] [TV4] EXITED LOOP: DrivingToPickUpPointWhileLoop.	Next, SPAWN'ing: (mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase04_PickUpPointAndBeyond)"];};//dbg
...
if (!_emergencyEscapeNeeded) then { 
	...
	if (_thisFileVerbosityLevelNumber>=2) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase03_CreateServiceUnitAndGoToRequestor.sqf] [TV2] <<<reached end-of-file>>>.   no emergency. proceeding with normal next phase in the workflow.			SPAWN'ing (mgmTfA_fnc_server_fixedDestinationTaxi_ServicePhase04_PickUpPointAndBeyond)."];};//dbg

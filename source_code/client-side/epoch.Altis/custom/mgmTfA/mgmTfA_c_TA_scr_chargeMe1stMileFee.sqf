//H
//H ~~
//H $FILE$		:	<mission>/custom/mgmTfA/mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf
...
//HH
//HH ~~
//HH	Syntax		:	_null = [] mgmTfA_scr_client_TA_chargeMe1stMileFee
//HH	Parameters	:	none
...
if (((vehicle player) getVariable ["mgmTfAisclickNGoTaxi", false])) then {
	...
	if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV8]     DETECTED     Player is currently in a clickNGoTaxi."];};
	...
	if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA]  [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV8] _myGUSUIDNumber has been obtained as: (%1)", (str _myGUSUIDNumber)];};
	...
	if (_thisFileVerbosityLevelNumber>=5) then {diag_log format ["[mgmTfA]  [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV4] This is _myGUSUIDNumber: (%1)		TOP OF FUNCTION EVALUATION 		(_TA1stMileFeeNeedToBePaidBool) is DETECTED: (%2)			", (str _myGUSUIDNumber), (str _TA1stMileFeeNeedToBePaidBool)];};
	...
	if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV8]          I have just obtained (_myVehiclesCommandingCustomerPlayerUIDNumber) as: (%1).		My PUID is: (%2).", (str _myVehiclesCommandingCustomerPlayerUIDNumber), (str _myPUID)];};
	...
	if (_myPUID == _myVehiclesCommandingCustomerPlayerUIDNumber) then {
		...
		if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV8]          YES, player is the commandingCustomer and mustPay the 1st Mile Fee.		_playerAuthorizedToPayBool  is set to true."];};
		_playerAuthorizedToPayBool = true;
	} else {
		if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV8]          NO, player is NOT the commandingCustomer and will not pay 1st Mile Fee.		_playerAuthorizedToPayBool  is set to false."];};
	...
	if (_playerAuthorizedToPayBool) then {
		if (_TA1stMileFeeNeedToBePaidBool) then {
			...
			if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV8]          YES, this vehicle is awaiting 1st Mile Fee payment.	"];};
		} else {
			// NO, this vehicle IS NOT awaiting 1st Mile Fee payment
			if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV8]          NO, this vehicle IS NOT awaiting 1st Mile Fee payment.	"];};
			private	[
					"_msg2HintTextString",
			...
			if (mgmTfA_configgv_clickNGoTaxisAbsoluteMinimumJourneyFeeInCryptoNumber > 0) then {
				...
				if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV3] 	DETECTED: 	DUPLICATE PAYMENT ATTEMPT		1st Mile Fee is ENABLED but does NOT NEED TO BE PAID now.		"];};
				...
				if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV3] 	DETECTED: 	PLAYER ATTEMPTED TO PAY UNNECESSARILY 		1st Mile Fee is NOT ENABLED on this server.		"];};
				// do 1stMileFeePayRequestor comms
				_msg2HintTextString = parsetext format ["<img size='6' image='custom\mgmTfA\img_comms\mgmTfA_img_client_warningStopSign.jpg'/><br/><br/><t size='1.40' color='#FF0037'>SORRY %1!<br/><br/>1ST MILE FEE<br/>IS DISABLED<br/><br/>", (profileName)];
		...
		if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV8]          NO, player is not authorized to pay the 1st Mile Fee"];};
	};
	...
	if (_playerAuthorizedToPayBool && _TA1stMileFeeNeedToBePaidBool) then {
		...
		if (_playerCashNumber >= mgmTfA_configgv_clickNGoTaxisAbsoluteMinimumJourneyFeeInCryptoNumber) then {
			...
			if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV8]          YES, player can afford the 1st Mile Fee	-- sending a request to the server to get charged.	"];};
			...
			// report to log
			if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV8]          CHARGED		the player TaxiAnywhere 1st Mile Fee cost	"];};
		} else {
			// NO, player cannot afford the 1st Mile Fee - log it
			if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV8]          NO, player cannot afford the next 1st Mile Fee.	let him & the server now.	"];};
			...
			if (_playerWentBankruptBool) then {if (mgmTfA_configgv_clientVerbosityLevel>=5) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV5]				I noticed (_playerWentBankruptBool) is  true!	"];};};
			...
			breakTo "mgmTfA_scr_client_TA_chargeMe1stMileFeeMainScope";
		};
	} else {
		// NO, player does not have to pay the 1st Mile Fee
		if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV8]          NO, player does not have to pay the 1st Mile Fee"];};
	};
} else {
	// NO, the player is NOT in a mgmTfAisclickNGoTaxi at the moment		-- log it, inform player & quit
	if (_thisFileVerbosityLevelNumber>=5) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_TA_chargeMe1stMileFee.sqf] [TV5]		DETECTED		Player currently IS NOT in a clickNGoTaxi!		The result of (str (((vehicle player) getVariable ['mgmTfAisclickNGoTaxi', false]))) is: (%1).", (str (((vehicle player) getVariable ["mgmTfAisclickNGoTaxi", false])))];};//dbg

...
if (_playerInTAVehicleBool) then {
	...
	if ((_vehSpeed == 0) && (!_doorsLockedBool)) then {
		...
		if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_c_TA_fnc_pleaseStopVehicle.sqf] [TV8]     DETECTED     We are in a bypass state.		DEVDEBUG"];};//DEVDEBUG

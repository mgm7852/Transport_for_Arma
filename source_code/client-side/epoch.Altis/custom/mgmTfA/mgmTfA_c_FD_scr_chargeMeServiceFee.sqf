//H
//H ~~
//H $FILE$		:	<mission>/custom/mgmTfA/mgmTfA_scr_client_FD_chargeMeServiceFee.sqf
...
//HH
//H ~~
//HH	Syntax		:	_null = [] mgmTfA_scr_client_FD_chargeMeServiceFee
//HH	Parameters	:	none
...
//HH		mgmTfA_dynamicgv_journeyServiceFeeCostInCryptoNumber	-- created by mgmTfA_fnc_client_sendBookingRequestFixedDestinationTaxi
//HH
...
if (((vehicle player) getVariable ["mgmTfAisfixedDestinationTaxi", false])) then {
	...
	if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV8]     DETECTED     Player is currently in a FixedDestinationTaxi."];};
	...
	if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA]  [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV8] _myGUSUIDNumber has been obtained as: (%1)", (str _myGUSUIDNumber)];};
	...
	if (_thisFileVerbosityLevelNumber>=5) then {diag_log format ["[mgmTfA]  [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV4] This is _myGUSUIDNumber: (%1)		TOP OF FUNCTION EVALUATION 		(_FD_serviceFeeNeedToBePaidBool) is DETECTED: (%2)			", (str _myGUSUIDNumber), (str _FD_serviceFeeNeedToBePaidBool)];};
	...
	if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV8]          I have just obtained (_myVehiclesCommandingCustomerPlayerUIDNumber) as: (%1).		My PUID is: (%2).", (str _myVehiclesCommandingCustomerPlayerUIDNumber), (str _myPUID)];};
	...
	if (_myPUID == _myVehiclesCommandingCustomerPlayerUIDNumber) then {
		...
		if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV8]          YES, player is the commandingCustomer and mustPay the Service Fee.		_playerAuthorizedToPayBool  is set to true."];};
		_playerAuthorizedToPayBool = true;
	} else {
		if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV8]          NO, player is NOT the commandingCustomer and will not pay Service Fee.		_playerAuthorizedToPayBool  is set to false."];};
	...
	if (_playerAuthorizedToPayBool) then {
		if (_FD_serviceFeeNeedToBePaidBool) then {
			...
			if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV8]          YES, this vehicle is awaiting Service Fee payment.	(and player is authorized to pay)	"];};
		} else {
			// NO, this vehicle IS NOT awaiting Service Fee payment
			if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV8]          NO, this vehicle IS NOT awaiting Service Fee payment.	"];};
			private	[
					"_msg2HintTextString",
			...
			if	(
				...
				if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV3] 	DETECTED: 	DUPLICATE PAYMENT ATTEMPT		Service Fee is ENABLED but does NOT NEED TO BE PAID now.		"];};
				...
				if (_thisFileVerbosityLevelNumber>=3) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV3] 	DETECTED: 	PLAYER ATTEMPTED TO PAY UNNECESSARILY 		Service Fee is NOT ENABLED on this server.		"];};
				// do ServiceFeePayRequestor comms
		...
		if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV8]          NO, player is not authorized to pay the Service Fee"];};
	};
	...
	if (_playerAuthorizedToPayBool && _FD_serviceFeeNeedToBePaidBool) then {
		...
		if (_playerCashNumber >= mgmTfA_dynamicgv_journeyServiceFeeCostInCryptoNumber) then {
			...
			if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV8]          YES, player can afford the Service Fee	-- sending a request to the server to get charged.	"];};
			...
			// report to log
			if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV8]          CHARGED		the player TaxiAnywhere Service Fee cost	"];};
		} else {
			// NO, player cannot afford the Service Fee - log it
			if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV8]          NO, player cannot afford the next Service Fee.	let him & the server now.	"];};
			...
			if (_playerWentBankruptBool) then {if (mgmTfA_configgv_clientVerbosityLevel>=5) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV5]				I noticed (_playerWentBankruptBool) is  true!	"];};};
			...
			breakTo "mgmTfA_scr_client_TA_chargeMe1stMileFee";
		};
	} else {
		// NO, player does not have to pay the Service Fee
		if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV8]          NO, player does not have to pay the Service Fee"];};
	};
} else {
	// NO, the player is NOT in a mgmTfAisfixedDestinationTaxi at the moment		-- log it, inform player & quit
	if (_thisFileVerbosityLevelNumber>=5) then {diag_log format ["[mgmTfA] [mgmTfA_scr_client_FD_chargeMeServiceFee.sqf] [TV5]		DETECTED		Player currently IS NOT in a FixedDestinationTaxi!		The result of (str (((vehicle player) getVariable ['mgmTfAisfixedDestinationTaxi', false]))) is: (%1).", (str (((vehicle player) getVariable ["mgmTfAisfixedDestinationTaxi", false])))];};//dbg

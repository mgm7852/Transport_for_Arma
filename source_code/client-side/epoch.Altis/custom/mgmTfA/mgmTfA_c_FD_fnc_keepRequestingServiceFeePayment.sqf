...
//H
//H ~~
//H $FILE$		:	<mission>/custom/mgmTfA/mgmTfA_fnc_client_FD_keepRequestingServiceFeePayment.sqf
...
//HH
//H ~~
//HH	Syntax		:	_null = [SU_ID] mgmTfA_fnc_client_FD_keepRequestingServiceFeePayment
...
//HH		mgmTfA_dynamicgv_journeyServiceFeeCostInCryptoNumber << contains a variable 'journey cost'. it was created by mgmTfA_fnc_client_sendBookingRequestFixedDestinationTaxi
...
scopeName "mgmTfA_fnc_client_FD_keepRequestingServiceFeePaymentMainScope";
...
if (_thisFileVerbosityLevelNumber>=5) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_client_FD_keepRequestingServiceFeePayment.sqf]  [TV5]	BEGIN RUNNING FUNCTION		I will  keep requesting Service Fee Payment till it is paid or phase timeout, for SU: (%1).", (str _originalVehiclesGUSUIDNumber)];};
...
while {_continueRequestingServiceFeePayment} do
{
	...
	if (_counterTen >= 10) then {
		if (_thisFileVerbosityLevelNumber>=8) then {diag_log format ["[mgmTfA] [mgmTfA_fnc_client_FD_keepRequestingServiceFeePayment.sqf]  [TV8] Reached checkpoint: Running main loop. Now executing the very top, just above sleep."];};
		_counterTen = 0;
	};
	// sleep a sec
	uiSleep 1;
	...
	if(_continueRequestingServiceFeePayment) then {
		...
		if (_thisFileVerbosityLevelNumber>=5) then {diag_log format ["[mgmTfA]  [mgmTfA_fnc_client_FD_keepRequestingServiceFeePayment.sqf] [TV5] This is _originalVehiclesGUSUIDNumber: (%1)		INSIDE LOOP EVALUATION 		(_continueRequestingServiceFeePayment) is: (%2)		I WILL CONTINUE LOOPING	", (str _originalVehiclesGUSUIDNumber), (str _continueRequestingServiceFeePayment)];};
		...
		if (_thisFileVerbosityLevelNumber>=5) then {diag_log format ["[mgmTfA]  [mgmTfA_fnc_client_FD_keepRequestingServiceFeePayment.sqf] [TV5] This is _originalVehiclesGUSUIDNumber: (%1)		INSIDE LOOP EVALUATION 		(_continueRequestingServiceFeePayment) is: (%2)		I WILL TERMINATE NOW	", (str _originalVehiclesGUSUIDNumber), (str _continueRequestingServiceFeePayment)];};
		// Exit the loops, go back to main, from where we will terminate AFTER writing to log.
		breakTo "mgmTfA_fnc_client_FD_keepRequestingServiceFeePaymentMainScope";
	};
};
if (_thisFileVerbosityLevelNumber>=9) then {diag_log format ["[mgmTfA]  [mgmTfA_fnc_client_FD_keepRequestingServiceFeePayment.sqf] [TV9] This is _originalVehiclesGUSUIDNumber: (%1)		THIS IS THE LAST LINE. TERMINATING.", (str _originalVehiclesGUSUIDNumber)];};
// EOF
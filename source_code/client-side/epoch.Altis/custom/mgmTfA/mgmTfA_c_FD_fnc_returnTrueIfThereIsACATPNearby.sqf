...
if (_numberOfCATPObjectsNearMe>0) then {
	// there are matching objects but is it just same class object or really a CATP?	check now
	{
		if (_x	getVariable ["mgmTfA_Dispatcher",false]) then {
			// we have a mgmTfA_Dispatcher here
			_returnValue = true;
...
if (_thisFileVerbosityLevelNumber>=10) then {diag_log format ["[mgmTfA] [mgmTfA_c_FD_fnc_returnTrueIfThereIsACATPNearby.sqf] [TV10] END reading file. The next line will return _returnValue and quit function. _returnValue is: (%1)", _returnValue];};//dbg
_returnValue;
// EOF
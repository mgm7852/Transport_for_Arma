		...
		if (!mgmTfA_dynamicgv_thisPlayerCanOrderTATaxiViaHotkey) then {
			if (isNil "mgmTfA_dynamicgv_lastTATaxiBookingPlacedAtTimestampInSecondsNumber") then {
				...
				if (mgmTfA_configgv_taxiAnywhereTaxisTaxiVehicleClassnameTextString == _classnameOfTheCurrentVehicle) then {
					...
					private ["_check7"];
					_check7 = ((vehicle player) getVariable "mgmTfAisTATaxi");
					if (!isNil "_check7") then {
						if (_thisFileVerbosityLevelNumber>=7) then {diag_log format ["[mgmTfA] [mgmTfA_c_TA_fnc_doLocalJanitorWork.sqf]  [TV7] check7 is not nil! player's vehicle does have the 'mgmTfAisTATaxi' variable attached to it, so it can be a TATaxi (we will need to check and see if this variable is set to 'true'). if it is 'true', this means he should not be able to order another clickNGo taxi at this time. we will NOT remove his inability to order taxis at this iteration. will loop and try again! Note: (str _check7) is: (%1)", (str _check7)];};
					} else {
